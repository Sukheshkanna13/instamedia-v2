...
export default function DatabaseExpansionPage() {
    ...
    const handleScrape = async () => {
        ...
        try {
            const res = await fetch("/api/database/scrape", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ keywords, platforms, limit: count })
            });
            if (!res.ok) throw new Error("Scraping endpoint not available yet (Phase 3)");
            const result = await res.json();

            setScrapedPosts(result.scraped_posts || []);

            if (result.mode === "mock") {
                setSuccess(`${result.message || "Scraped successfully"} (Demo mode - using sample data)`);
            } else {
                setSuccess(result.message || `Scraped ${result.scraped_posts?.length || 0} posts`);
            }
        } catch (e) {
    ...
    const handleAddSelected = async () => {
        ...
        try {
            ...
            const res = await fetch("/api/database/add", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ posts: postsToAdd })
            });
            if (!res.ok) throw new Error("Database expansion endpoint not available yet (Phase 3)");
            const result = await res.json();

            setSuccess(`✅ ${result.message || "Added successfully"}! Total posts in database: ${result.total_posts || "?"}`);
            setScrapedPosts([]);
            setSelectedPosts(new Set());
            await loadStats();
        } catch (e) {
